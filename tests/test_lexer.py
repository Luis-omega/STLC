...
from STLC.Parser.Lexer import TokenVariable, variable, TokenInt, int_,Operator, operator

import pytest

T = TypeVar("T")

def make_positive_test(stream:str, lexer_function : Callable[[str],Optional[tuple[str,T]]]
    , expected:T)->None:
...
def make_negative_test(stream:str, lexer_function : Callable[[str],Optional[tuple[str,T]]])->None:
    lexed = lexer_function(stream)
